function Compress-Vhd {
    <#
.Synopsis
   Compresses all the VHD files from a specified location
.DESCRIPTION
   Compress-Vhdx retrieves each VHD files from a specified location and compacts each using the native command Optimize-VHD
.EXAMPLE
...
#>
    ...
    Begin {
        ...
        if ($IncludeSubfolders) {
            $AllVhdx = Get-ChildItem *.vhd -Path $Path -Recurse -ErrorAction SilentlyContinue
        }
        else {
            $AllVhdx = Get-ChildItem *.vhd -Path $Path -ErrorAction SilentlyContinue
        }
        ...
        #Clear-Host
        Write-Verbose "Compacting $($AllVhdx.Count) VHD files, please wait"
    } #Begin
...
# Compress VHD Fils
Compress-Vhd -Path D:\Hyper-V\ -IncludeSubfolders -Verbose