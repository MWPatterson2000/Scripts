...
$DCList = Get-ADDomainController -Filter * -Server $DomainName | Select-Object Hostname, Name

# Check Domain Controllers
foreach ($DC in $DCList) {
	Test-ServerRolePortGroup -Computername $DC.Hostname -ServerRole WinRm, SMB, DNS, ActiveDirectoryGeneral, ActiveDirectoryGlobalCatalog, NetBios, RDP | Format-Table -AutoSize
}
